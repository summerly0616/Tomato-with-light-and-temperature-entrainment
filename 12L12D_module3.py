...
def tomato_circadian(y,t):
    ...
    z=np.array([(v1+v1L*L*P)/(1+(CLp/K0)**2+(P97p/K1)**2)-(k1L*L+k1D*D)*CLm,\
                (p1+p1L*L)*CLm-d1*CLp,\
                (v2L*L*P+v2A)/(1+(CLp/K3)**2+(ELp/K5)**2)-k2*P97m,\
                p2*P97m-(d2D*D+d2L*L)*P97p,\
                (v4+L*v4L)/(1+(CLp/K8)**2+(ELp/K10)**2)-k4*ELm,\
...
y0 = [0.73504743, 1.13705128, 0.59862813, 0.74156027,  0.07109132, 0.07213094, 0.69340948]
...
plt.plot(t-72,yy1[:,0],'k-',label='Continuous',linewidth=3)
plt.plot(t-72,yy2[:,0],'b-',label='Discrete',linewidth=3)

plt.xlabel('Time(h)',font)
plt.ylabel('Relative $\it{CL}$ mRNA level',font1)
plt.title('module3',font)
